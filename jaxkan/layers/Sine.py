...
class SineLayer(nnx.Module):
    ...
    def update_degree(self, x, degree_new):
        """
        Update the number of basis functions for SineLayer.
        
        For the case of sine-based KANs there is no concept of grid. However, a 
        fine-graining approach can be followed by progressively increasing the 
        number of basis functions (and by extension phases and omegas). This 
        method increases the count while preserving the function approximation 
        ...
        self.omega = nnx.Param(
            nnx.initializers.normal(stddev = 1.0)(
                self.rngs.params(), (degree_new, 1), jnp.float32)
        )

        self.phase = nnx.Param(jnp.zeros((degree_new, 1)))

        # Get the Bj(x) for the new degree order
        Bj = self.basis(x).transpose(1, 0, 2) # (n_in, batch, degree_new)

        # Solve for the new coefficients
        cj = solve_full_lstsq(Bj, ciBi) # (n_in, degree_new, n_out)
        # Cast into shape (n_out, n_in, degree_new)
