...
class FourierLayer(nnx.Module):
    ...
    def update_degree(self, x, degree_new):
        """
        Update the Fourier series order for FourierLayer.
        
        For the case of FourierKAN there is no concept of grid. However, a 
        fine-graining approach can be followed by progressively increasing 
        the number of summands. This method increases the order while
        ...
        cj, sj = self.basis(x) # (batch, n_in, degree_new)
        cj, sj = cj.transpose(1, 0, 2), sj.transpose(1, 0, 2) # (n_in, batch, degree_new)

        # Solve for the new cosine coefficients
        new_cos_w = solve_full_lstsq(cj, cosines) # (n_in, degree_new, n_out)
        
        # Solve for the new sine coefficients
        new_sin_w = solve_full_lstsq(sj, sines) # (n_in, degree_new, n_out)
