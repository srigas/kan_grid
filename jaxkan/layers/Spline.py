...
from .utils import solve_full_lstsq_batched


class SplineLayer(GridAdaptiveMixin, nnx.Module):
    """
    ...
    def __init__(self,
        ...
        """
        Initializes a SplineLayer instance.
        ...
        Example:
            ...
            >>>                     G = 3, grid_range = (-1,1), residual = nnx.silu,
        ...
        # Convert grid_range tuple to bounds array of shape (n_in, 2)
        # Ensure float32 dtype to avoid scatter warnings with integer grid_range
        bounds = jnp.stack([
            jnp.full((n_in,), grid_range[0], dtype=jnp.float32),
            jnp.full((n_in,), grid_range[1], dtype=jnp.float32)
        ], axis=1)
        self.grid = SplineGrid(n_in=n_in, k=k, G=G, bounds=bounds)
    ...
    def basis(self, x):
        ...
        grid = self.grid.knots  # shape (n_in, G+2k+1)
    ...
    # =========================================================================
    # GridAdaptiveMixin interface implementation
    # =========================================================================
    
    def _get_grid_values(self) -> jnp.ndarray:
        """Get the current knot values."""
        return self.grid.knots
    
    def _set_grid_values(self, dim: int, values: jnp.ndarray) -> None:
        """Set interior knots for a dimension (augmentation handled by grid)."""
        self.grid.set_knots(dim=dim, interior_knots=values)
    
    def _get_num_grid_points(self) -> int:
        """Get number of interior knots (G+1)."""
        return self.grid.G + 1
    
    def _update_grid_size(self, new_size: int) -> None:
        """Update G (number of intervals)."""
        if new_size != self.grid.G:
            self.grid.update_G(new_size)
    
    def _compute_basis_output(self, x: jnp.ndarray) -> jnp.ndarray:
        """Compute y = Sum(c_i * B_i(x)) for coefficient preservation."""
        Bi = self.basis(x).transpose(1, 0, 2)  # (n_in, batch, G+k)
        ci = self.c_basis[...].transpose(1, 2, 0)  # (n_in, G+k, n_out)
        return jnp.einsum('ijk,ikm->ijm', Bi, ci)  # (n_in, batch, n_out)
    
    def _recompute_coefficients(self, x: jnp.ndarray, target_output: jnp.ndarray) -> None:
        """Solve least squares for new coefficients after grid change."""
        Bj = self.basis(x).transpose(1, 0, 2)  # (n_in, batch, G_new+k)
        cj = solve_full_lstsq_batched(Bj, target_output)  # (n_in, G_new+k, n_out)
        cj = cj.transpose(2, 0, 1)  # (n_out, n_in, G_new+k)
    ...
    def __call__(self, x):
        ...
        Example:
            >>> layer = SplineLayer(n_in = 2, n_out = 5, k = 3, G = 5,
            >>>                       grid_range = (-2.0, 2.0), residual = None,
