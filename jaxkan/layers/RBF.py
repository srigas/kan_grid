...
class RBFLayer(GridAdaptiveMixin, nnx.Module):
    ...
    def __init__(self, n_in: int = 2, n_out: int = 5, D: int = 5, kernel: Union[dict, None] = None,
        ...
        # Initialize the grid - shape (n_in, D)
        ...
        self.grid = RBFGrid(n_in=n_in, D=D, bounds=bounds)
    ...
    def basis(self, x):
        ...
        grid_centers = self.grid.centers

        # Gaussian kernel
        if kernel_type == "gaussian":
            std = self.kernel.get("std", 1.0)
            
            rbf = jnp.exp(-0.5 * ((x[..., :, None] - grid_centers[None, ...]) / std) ** 2)
    ...
    def _get_grid_values(self) -> jnp.ndarray:
        """Get the current center values."""
        return self.grid.centers
    
    def _set_grid_values(self, dim: int, values: jnp.ndarray) -> None:
        """Set centers for a dimension."""
        self.grid.set_centers(dim=dim, centers=values)
    
    def _get_num_grid_points(self) -> int:
        """Get number of centers (D)."""
        return self.grid.D
    
    def _update_grid_size(self, new_size: int) -> None:
        """Update D (number of centers)."""
        if new_size != self.grid.D:
            self.grid.update_D(new_size)
            self.D = new_size
    ...
    def _recompute_coefficients(self, x: jnp.ndarray, target_output: jnp.ndarray) -> None:
        ...
        cj = solve_full_lstsq_batched(Bj, target_output)  # (n_in, D_new, n_out)
        cj = cj.transpose(2, 0, 1)  # (n_out, n_in, D_new)
