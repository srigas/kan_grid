...
def normalize_to_list(value: Any, num_layers: int, param_name: str) -> List[Any]:
    """
    Normalize a parameter to a list with one element per layer.
    
    If a single value is provided, it is replicated for all layers.
    If a list is provided, its length must match num_layers.
    ...
    -------
    List[Any]
        List with one element per layer.
    
    Raises
    ------
    ValueError
        If a list is provided with incorrect length.
        
    Examples
    --------
    >>> # Single value replicated for all layers
    >>> normalize_to_list(0.1, 3, "learning_rate")
    [0.1, 0.1, 0.1]
    
    >>> # List passed through if length matches
    >>> normalize_to_list([0.1, 0.2, 0.3], 3, "learning_rate")
    [0.1, 0.2, 0.3]
    
    >>> # None becomes list of Nones
    >>> normalize_to_list(None, 3, "optional_param")
    [None, None, None]
    """
    if value is None:
    ...
    if isinstance(value, list):
        if len(value) != num_layers:
            raise ValueError(
                f"{param_name} list has {len(value)} elements, but model has {num_layers} layers. "
                f"Provide either a single value (applied to all layers) or a list with {num_layers} elements."
            )
        return value
    
    # Single value - replicate for all layers
